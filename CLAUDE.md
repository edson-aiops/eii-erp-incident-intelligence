...
EII is a **multi-HCM AIOps application** for resolving incidents in HCM and ERP systems. It is deployed to HuggingFace Spaces via Docker.
...
- `HCMRouter` — detects system (HCM/ERP), module, and language
...
26 ChromaDB collections under `data/kb/`, named `{system}_{module}_{lang}` (e.g., `hcm_payroll_pt`, `hcm_esocial_pt`, `hcm_pro_benefits_en`). The `data/` directory is mounted in Docker and kept empty via `.gitkeep` until Phase 2.
