...
│   │                       │         │                        │ │
│   │  Edson + ProSecurity  │         │  Recrutadores + público │ │
...
**Quando usar:** desenvolvimento, demonstração para ProSecurity, testes com dados reais, validação do caminho LGPD.
...
Adicionar uma feature interna (ex: novo painel admin, integração com SAR2G real) só vai em `app.py`. Não tente "portar" para `app_hf.py` se não fizer sentido publicamente.
